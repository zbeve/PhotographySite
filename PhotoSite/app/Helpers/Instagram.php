...
class Instagram {
  ...
  private $_actions = array('follow', 'unfollow', 'block', 'unblock', 'approve', 'deny');
	...
	{
		if ($config->has('instagram::config'))
		{
			$this->tconfig = $config->get('instagram::config');
		}
		else if ($config->get('instagram'))
		{
			$this->tconfig = $config->get('instagram');
		}
		else
		{
		...
		$this->parent_config                    = [];
		$this->parent_config['client_id']    = $this->tconfig['CLIENT_ID'];
		$this->parent_config['clien_secret'] = $this->tconfig['CLIENT_SECRET'];
		$this->parent_config['redirect_uri']           = $this->tconfig['REDIRECT_URI'];

    // if you want to access user data
    $this->setApiKey($this->parent_config['client_id']);
    $this->setApiSecret($this->parent_config['clien_secret']);
    $this->setApiCallback($this->parent_config['redirect_uri']);
	}
